...
export function punish(ni: NetworkIdentifier, cheat: Cheats): void {
    serverInstance.disconnectClient(ni, `당신은 밴되었습니다! by §l§4테러방지§r due to cheating: ${cheat}`);
    // Add your own punishmenet here
    console.log(`${"[테러방지]".red} ${ni.getActor()?.getName()} was detected for: ${cheat}`);
}

export function report(name: string, reason: string): void {
    console.log(`${"[테러방지]".red} ${name} was reported for: ${reason}`);
}
